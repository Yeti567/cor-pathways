...
import { AlertTriangle, ArrowLeft, BadgeCheck, IdCard, Save, UserRound } from "lucide-react";
import { createTransportDriver } from "@/app/admin/actions";
import { AdminShell } from "@/app/admin/_components/AdminShell";
import { canUseAdminPanel } from "@/lib/access-control";
import { requireAppUser } from "@/lib/current-user";
import { driverDeficiencies, type TransportDocumentRecord } from "@/lib/transport-registry";
...
export default async function TransportDriversPage({ searchParams }: DriversPageProps) {
  ...
  const driverRows = (drivers ?? []).map((driver) => {
    const deficiencies = driverDeficiencies(documentsByDriver.get(driver.id) ?? []);
    return { driver, deficiencyCount: deficiencies.length };
  });
  const compliantCount = driverRows.filter((row) => row.deficiencyCount === 0).length;
    ...
    <AdminShell eyebrow="Transport" tenantName={context.tenant?.name ?? "Company profile"} title="Drivers">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-bold text-[var(--ink)]">{driverRows.length - compliantCount}</p>
        </div>
      </div>
      ...
      <section className="mt-5 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-sm">
        ...
        {driverRows.length > 0 ? (
          <div className="divide-y divide-[var(--border)]">
            {driverRows.map(({ driver, deficiencyCount }) => (
              <Link
